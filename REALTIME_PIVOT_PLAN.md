...
### M3 — 波次與內容（1.5～2 週）← **這一輪只做了核心玩法那半，範圍見下**
本輪刻意縮小到「即時制本身好不好玩」這一半：波次生成、敵人多樣性、Boss。
「接進正式地圖流程」跟「遺物系統」評估後太大，明確留給下一輪，見文末待辦。
...
- ⛔ **未做，明確留給下一輪**：
  - 遺物系統接回（波次/Boss 結算掉落）——跟 M2 的 buffCards 決策一樣，
    `relics.ts` 531 行也大多是回合制專用效果，接回去要嘛比照 cards.ts 重
    寫一套精簡版，要嘛做一層轉譯層，這是下一輪的決定點，不是本輪份內
  - 節點地圖簡化版接上 `ArenaScreen`（目前還是掛在 GM 模式的測試入口，
    不是真的遊戲流程）——這是「這遊戲能不能真的玩一整輪」的關鍵一步，
    份量不小（要動 `App.tsx` 的 `battle` phase 分支、地圖生成、結算畫面），
    值得單獨開一輪做，不要跟波次系統的驗證擠在一起
