...
export async function onRequestPost({ request, env }) {
  ...
  for (const row of results) {
    try {
      ...
      if (dungeonWins === 0) { skipped++; continue }

      // 補上副本通關次數（不重複計算，只加還沒計入的）
      const heroProgress = meta.heroProgress ?? {}
      const updatedProgress = {}
      for (const [heroId, prog] of Object.entries(heroProgress)) {
        const newRunsWon       = (prog.runsWon       ?? 0) + dungeonWins
        const newRunsCompleted = (prog.runsCompleted  ?? 0) + dungeonWins
        const patched = { ...prog, runsWon: newRunsWon, runsCompleted: newRunsCompleted }
        const newStars = Math.max(patched.stars ?? 0, checkStarConditions(patched))
      ...
      report.push({ uid: row.uid, dungeonWins })
      updated++
    } catch (e) {
      skipped++
    }
  ...
  return new Response(
    JSON.stringify({ ok: true, total: results.length, updated, skipped, report }),
