/**
 * 機關技師模組匯入（2026-08-18）：跟 scripts/import-firemage-redo.mjs 同一套
 * 做法——來源已經是乾淨透明底（sprite-info.json 標記 sourceMotherSheet 為
 * 「透明母圖」，實測四角像素 alpha=0 確認），只要裁切到內容 bounding box
 * 就好，不用 process-hero-frames.mjs 那套 flood-fill 去背。目前部署的機關
 * 技師只有一張佔位 idle_0.png（其餘狀態全靠 fallback），這次是補齊完整
 * 20 格逐幀動畫。
 *
 * 執行：node scripts/import-engineer-redo.mjs
 */
...
const SRC_DIR = 'D:/CLAUDE專案/三選一/英雄圖/機關技師/機關技師/individual'
const SRC_PREFIX = 'mechanic_engineer'
const OUT_DIR = 'public/assets/frames/heroes/engineer/s0'
