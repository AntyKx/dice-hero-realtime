/**
 * 火焰法師模組重做（2026-08-18）匯入：來源已經是乾淨透明底（sprite-info.json
 * 標記 sourceMotherSheet 為「透明母圖」，實測四角/邊緣像素 alpha=0 確認），
 * 不需要 process-hero-frames.mjs 那套 flood-fill 去背——只要裁切到內容
 * bounding box（留一點 padding）就好，做法沿用同一支腳本的 trimToContent()
 * 邏輯，維持跟現有 mage 素材一致的處理方式。
 *
 * 執行：node scripts/import-firemage-redo.mjs
 */
...
const SRC_DIR = 'D:/CLAUDE專案/三選一/英雄圖/火焰法師/火焰法師/individual'
const SRC_PREFIX = 'fire_mage'
...
console.log('開始匯入火焰法師重做版 20 張逐幀圖...\n')
